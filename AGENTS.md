...
*   **Notifications**: We utilize `expo-notifications` for scheduled local notifications like the daily reminder scheduled globally in `App.js` for 8:00 AM, and the testing "Lembrete Teste 5s" shortcut in `HomeScreen.js`. For immediate UI interaction feedback (like favoriting a priest/church, setting a confession or visit reminder, reading news, or clearing filters), we exclusively use `useToast()` or handle UI state directly to avoid UX anti-patterns associated with system-level push notifications.
