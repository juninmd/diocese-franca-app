...
*   **Scraper**: A scraper uses `axios` and `cheerio` to extract the latest Diocese data (`backend/scraper.js`). Updates here must be parsed carefully according to the live DOM. Note: When scraping the website, it uses the class selector `.section_post_left` to accurately fetch news cards and prevent missing content elements reliably from the markup. It extracts title, category (from `.buttons_container a.banner_button`), link, image (with `data-src` fallback, and a placeholder fallback url if no image is present), description (`.post_text p`, cleaning up formatting and extra whitespace), and date (from `.event_date`, `.date`, `.post_date`, or utilizing an expanded regex to extract from title, description, or image URLs like `images/20260812223051`), checking against duplicate links before inserting. We rely on robust explicit fallbacks (e.g., extracting from image paths, falling back to `h2.post_title` directly if the `a` tag is missing, enforcing absolute URLs with `URL()` constructor, trimming spacing inside elements) logging detailed information for debug before triggering fallbacks, and increased request timeouts (e.g. 20000ms) to maintain stability. A retry loop is built into the initial request to handle intermittent connection failures up to 3 times before returning an empty JSON array fallback. The scraper uses an array of random `User-Agent` strings, as well as `Accept`, `Accept-Language`, and `Referer` headers to look more like a browser request and prevent blocking. It logs response statuses on errors within the retry loop for better debugging. Scraper script now runs dynamically when required using `require.main === module` for clean inline execution instead of infinite interval loop.
...
*   **Notifications**: We utilize `expo-notifications` for scheduled local notifications like the daily reminder scheduled globally in `App.js` for 8:00 AM, contextual prompts like "Agendar Visita" in the paróquias screen (`ChurchDetailScreen.js`), the "Lembrete Confissão" button in the priests screen (`PriestDetailScreen.js`), reminders for masses in the mass screen (`MassesScreen.js`), and the testing "Lembrete Teste 5s" shortcut in `HomeScreen.js`. For immediate UI interaction feedback (like favoriting a priest/church, reading news, or clearing filters), we exclusively use `useToast()` or handle UI state directly to avoid UX anti-patterns associated with system-level push notifications. However, explicit local notification scheduling is supported and implemented for features like "Lembrete Confissão" and Missa reminders.
*   **UI/UX States**: Keep empty states helpful and empathetic to guide the user (e.g., "Puxa, não conseguimos carregar as igrejas agora. Tente novamente!" instead of "Erro ao carregar"). Changed network errors and empty search states to use empathetic texts, standardizing the "Puxa, ..." prefix across all screens and detailed screens. Touchables in list components and detail screens should have `activeOpacity={0.8}`. Ensure widths fit well on various screen scales.
