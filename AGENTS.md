...
*   **Notifications**: We utilize `expo-notifications` for scheduled local notifications like the daily reminder scheduled globally in `App.js` for 8:00 AM, contextual prompts like "Agendar Visita" in the paróquias screen (`ChurchDetailScreen.js`), and the testing "Lembrete Teste (5s)" shortcut in `HomeScreen.js`. For immediate UI interaction feedback (like favoriting a priest/church, reading news, or clearing filters), we exclusively use `useToast()` or handle UI state directly to avoid UX anti-patterns associated with system-level push notifications. However, explicit local notification scheduling is supported and implemented for features like "Lembrete Confissão" and Missa reminders.
*   **UI/UX States**: Keep empty states helpful and empathetic to guide the user (e.g., "Puxa, não conseguimos carregar as igrejas agora. Tente novamente!" instead of "Erro ao carregar"). Changed network errors and empty search states to use empathetic texts, standardizing the "Puxa, não conseguimos..." e "Puxa, não encontramos..." prefix across all screens and detailed screens. Touchables in list components and detail screens should uniformly have `activeOpacity={0.8}` to provide correct tap feedback. Ensure widths fit well on various screen scales.
