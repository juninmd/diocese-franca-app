...
- **Notificações Locais (Lembrete Diário/Testes)**: Implementação de lembretes diários (8h da manhã) com `expo-notifications` para incentivo à oração, configurado globalmente em `App.js` na inicialização, e um novo botão de teste ("Lembrete Teste 5s") presente no Acesso Rápido na Home. Além disso, a opção "Agendar Visita" nas paróquias, "Lembrete Confissão" nos padres e Lembretes de Missa disparam uma notificação local contextual.
